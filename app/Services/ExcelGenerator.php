...
class ExcelGenerator
{
    ...
    public function createProfitFirstWorkbook(array $data): string
    {
        try {
            ...
            // Save file
            $filename = 'profit-first/profit_first_' . time() . '_' . uniqid() . '.xlsx';
            $writer = new Xlsx($spreadsheet);

            // Ensure directory exists
            $directory = 'profit-first';
            ...
            // Return relative path (not URL) so controller can use Storage::disk('public')->download()
            return $filename;
        } catch (\Exception $e) {
            \Log::error('Excel generation error', [
                'error' => $e->getMessage(),
